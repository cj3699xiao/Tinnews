...
public class NewsRepository {
    ...
    public LiveData<Boolean> favoriteArticle(Article article) {
            // need return to implement toast
    ...
    public LiveData<List<Article>> getAllSavedArticles() {
            // thwn read, there will be a early copy to read, when there is
        // still write ops
        return database.articleDao().getAllArticles();
    }

    public void deleteSavedArticle(Article article) {
            // same format as favorite article, but without return
    ...
    private static class FavoriteAsyncTask extends AsyncTask<Article, Void, Boolean> {
        ...
        protected Boolean doInBackground(Article... articles) {
            //varargs arguments could be one or more !  <-  ...
            Article article = articles[0];
            try {
                database.articleDao().saveArticle(article);
                //desk io, do in background
            } catch (Exception e) {
                return false;
            }
            return true;
